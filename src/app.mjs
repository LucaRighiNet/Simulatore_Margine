...
const LIVELLI = [
  ['base', 'Base', 'Ricavo, materiale e manodopera per linea. Nove numeri in tutto.'],
...
function pannelloParametri() {
  const campi = [campoNumero('Margine industriale obiettivo %', 'parametri.targetPct', sim.parametri.targetPct)];
  if (livello !== 'base') {
    campi.unshift(
  ...
  if (livello === 'completo') {
    campi.push(campoTesto('Tariffe valide dal', 'parametri.validitaTariffe', sim.parametri.validitaTariffe, { type: 'date' }));
  }
  ...
  return h('section', { class: 'pannello' },
    ...
    h('div', { class: 'corpo' },
      ...
      livello === 'base'
        ? h('p', { class: 'nota' },
          'Spese generali e contingency restano a zero a questo livello, quindi il margine industriale coincide con il margine di contribuzione. ',
          pulsanteEspandi('intermedio', 'Aggiungili'))
        : null,
      livello === 'completo' ? h('div', { style: 'margin-top:12px' }, tabellaTariffe()) : null,
      livello === 'intermedio'
  ...
  if (voce.cat === 'manodopera') {
    ...
    return h('td', { class: 'n', 'data-et': ETICHETTA_COLONNA[colonna] },
      h('div', { class: 'coppia' },
        ...
        h('span', { class: 'suff', testo: ore ? 'h' : '\u20ac' })));
  }
  if (voce.cat === 'materiale') {
    return h('td', { class: 'n', 'data-et': ETICHETTA_COLONNA[colonna] },
      h('div', { class: 'coppia' },
        inputNumero(`${percorsoBase}.${colonna}.q`, voce[colonna].q, { placeholder: 'listino', title: 'Importo di listino' }),
        inputNumero(`${percorsoBase}.${colonna}.sconto`, voce[colonna].sconto, { placeholder: '0', style: 'width:52px', title: 'Sconto percentuale sul listino' }),
...
function rigaVoce(linea, iL, voce, iV) {
  ...
  if (voce.cat === 'manodopera') {
    const ore = modoManodopera(voce) === 'ore';
    celleNome.push(h('div', { class: 'coppia', style: 'margin-top:4px;gap:6px' },
      h('button', {
  ...
  return h('tr', {},
    ...
    h('td', { class: 'n derivato', 'data-et': 'Simulato', 'data-out': 'voce:' + voce.id, testo: '\u2014' }),
    h('td', { class: 'azione-riga' }, h('button', {
...
function tabellaLineaDettagliata(linea, iL) {
  ...
  corpo.appendChild(h('tr', { class: 'somma' },
    h('td', { 'data-et': 'Voce', testo: 'Ricavo di linea' }),
    ...cols.map((c) => h('td', { class: 'n', 'data-et': ETICHETTA_COLONNA[c] },
      inputNumero(`linee.${iL}.ricavo.${c}`, linea.ricavo[c], { placeholder: '0' }))),
    h('td', { class: 'n derivato', 'data-et': 'Simulato', 'data-out': 'ricavo:' + linea.id, testo: '\u2014' }),
  ...
  return h('div', { class: 'tabellone' }, h('table', { class: 'tab-dati' },
    h('thead', {}, h('tr', {},
      h('th', { testo: 'Voce' }),
  ...
  for (const cat of categorieVisibili(linea)) {
    ...
    const celle = cols.map((c) => (unica
      ? h('td', { class: 'n', 'data-et': ETICHETTA_COLONNA[c] },
        inputNumero(`linee.${iL}.voci.${iV}.${c}.q`, unica[c].q, { placeholder: '0' }))
...
function pannelloLinea(linea, iL) {
  return h('section', { class: 'pannello' },
    h('div', { class: 'linea-titolo' },
      ...
      h('button', {
        class: 'muto', title: 'Rimuovi la linea di servizio', 'aria-label': 'Rimuovi la linea di servizio', testo: '\u00d7',
...
function schedaGuida() {
  ...
  return h('div', { class: 'guida' },
    h('section', { class: 'pannello' },
      ...
      h('div', { class: 'corpo' },
        h('p', {}, 'Simula come si muove il margine di una commessa al variare dei costi, divisi per linea di servizio. Non è un consuntivo e non tiene lo storico: ogni simulazione è una fotografia del momento in cui la fai. Quando serve, la salvi in archivio e non ci torni più sopra.'),
        h('p', {}, 'Tre colonne sulla stessa struttura: ',
          ...
          h('b', { testo: 'Simulato' }), ' (lo scenario che stai provando). Il simulato si ottiene muovendo i cursori, non riscrivendo i numeri.'))),

    h('section', { class: 'pannello' },
      h('h2', { testo: 'Inizia semplice, aggiungi dettaglio dopo' }),
      h('div', { class: 'corpo' },
        h('p', {}, 'Lo strumento si apre al livello Base: per ogni linea di servizio inserisci ricavo, materiale e manodopera. Nove numeri e hai un margine. Quando ti serve di più, i pulsanti nella scheda Dati espandono la maschera senza perdere nulla di quanto inserito.'),
        vociGuida([
          ['Base', 'Ricavo, materiale e manodopera per linea, una colonna sola. Per una stima rapida, anche dal telefono.'],
    ...
    h('section', { class: 'pannello' },
      ...
      h('div', { class: 'corpo' },
        vociGuida([
          ['Margine di contribuzione', 'Ricavo meno costi diretti della commessa. È la leva che il PM controlla.'],
          ['Margine industriale', 'Dopo spese generali e contingency. È quello che si confronta con il budget aziendale.'],
...
function scrivi(sel, testo, classe, spiegazione) {
...
function aggiornaDerivati() {
  ...
  for (const c of COLONNE) {
    ...
    scrivi(`tot:${c}:speseGenerali:euro`, formatta(col.speseGenerali, 'euro'), null,
      `Costi diretti ${euro(col.cd)} \u00d7 ${pctTesto(sg)} = ${euro(col.speseGenerali)}.`);
    scrivi(`tot:${c}:contingency:euro`, formatta(col.contingency, 'euro'), null,
      `Costi diretti ${euro(col.cd)} \u00d7 ${pctTesto(ctg)} = ${euro(col.contingency)}.`);
    scrivi(`tot:${c}:mi:euro`, formatta(col.mi, 'euro'), null,
      `${euro(col.ricavo)} meno costi ${euro(col.cd)} meno spese generali ${euro(col.speseGenerali)} meno contingency ${euro(col.contingency)} = ${euro(col.mi)}.`);
  ...
  for (const linea of sim.linee) {
    ...
    for (const voce of linea.voci) {
      ...
      scrivi('voce:' + voce.id, euro(costo), null, parti.join(' '));
    }
  }
  ...
  for (const [chiave, g] of [['gapPreventivoKom', r.gapPreventivoKom], ['gapKomSimulato', r.gapKomSimulato]]) {
    ...
    if (tb) {
      ...
      for (const l of g.perLinea) {
        tb.appendChild(h('tr', {},
          h('td', { 'data-et': 'Linea', testo: l.nome }),
  ...
  scrivi('tot:simulato:cdMax:euro', euro(s.cdMax), null,
    `Ricavo ${euro(s.ricavo)} \u00d7 (100% meno obiettivo ${perc(target)}) diviso (1 + spese generali ${pctTesto(sg)} + contingency ${pctTesto(ctg)}) = ${euro(s.cdMax)}.`);
